...
exports.PLATFORM_METADATA = exports.Platform = void 0;
exports.detectPlatform = detectPlatform;
exports.getPlatformMetadata = getPlatformMetadata;
/**
...
(function (Platform) {
     ...
     */
    Platform["SPRING_BOOT"] = "spring-boot";
    /**
 ...
 */
function detectPlatform(filePaths) {
    const fileNames = filePaths.map(path => {
        const parts = path.split('/');
        return parts[parts.length - 1] || '';
    });
    // Check Spring Boot first (most specific)
    for (const metadata of exports.PLATFORM_METADATA) {
        if (metadata.platform === Platform.GENERIC) {
            continue; // Skip generic, check last
        }
        const hasMatch = fileNames.some(fileName => fileName && metadata.filePatterns.some(pattern => fileName.includes(pattern)));
        if (hasMatch) {
...
function getPlatformMetadata(platform) {
    const metadata = exports.PLATFORM_METADATA.find(m => m.platform === platform);
    if (!metadata) {
