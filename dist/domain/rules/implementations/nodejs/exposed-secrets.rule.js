 ...
 */
exports.exposedSecretsRule = {
    id: 'EXPOSED_SECRETS',
    name: 'Weak or Placeholder Secrets',
    description: 'Detects secrets with weak, default, or placeholder values. ' +
    ...
    evaluate(entry) {
        ...
        const isSecretKey = SECRET_KEY_PATTERNS.some(pattern => entry.key.toLowerCase().includes(pattern));
        if (!isSecretKey) {
            return [];
        }
        ...
        if (isWeak) {
            return [
                (0, violation_1.createViolation)({
                    ...
                    message: `Secret "${entry.key}" has a weak or placeholder value. ` +
                        `This is a critical security risk.`,
                    filePath: entry.sourceFile,
                    configKey: entry.key,
                    configValue: '***REDACTED***', // Don't expose the actual secret
                    lineNumber: entry.lineNumber,
                    suggestion: `Generate a strong, random secret (at least 32 characters). ` +
...
//# sourceMappingURL=exposed-secrets.rule.js.map